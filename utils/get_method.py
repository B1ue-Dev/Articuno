...
class Get_method(interactions.Extension):
    ...
    async def _get_user(self, ctx: interactions.CommandContext, id: str):
        user = interactions.User(
            **await self.bot._http.get_user(id), _client=self.bot._http
        )
    ...
    async def _get_member(self, ctx: interactions.CommandContext, id: str):
        member = interactions.Member(
            **await self.bot._http.get_member(ctx.guild_id, id), _client=self.bot._http
        )
        ...
        await ctx.send(
            content="{}#{} ({})\n{}\nJoined at: <t:{}>\nCreated at: <t:{}>\nNick: {}".format(
                username, discriminator, id, avatar_url, joined_at, created_at, nick
            )
        )
    ...
    async def _get_guild(self, ctx: interactions.CommandContext):
        guild = interactions.Guild(
            **await self.bot._http.get_guild(ctx.guild_id), _client=self.bot._http
        )
        user = interactions.User(
            **await self.bot._http.get_user(guild.owner_id), _client=self.bot._http
        )
        username = user.username
        discriminator = user.discriminator
        name = guild.name
        ...
        await ctx.send(
            content="{} ({})\n{}\nOwner: {}#{} ({})".format(
                name, id, icon_url, username, discriminator, guild.owner_id
            )
        )
