...
class Admin(interactions.Extension):
    ...
    async def _user(self, ctx: interactions.CommandContext, sub_command: str,
        ...
        if sub_command == "kick":
            ...
            bot = await guild.get_member(member_id=self.client.me.id)
            role = bot.roles[0]
            ...
            await self.client._http.create_guild_ban(guild_id=ctx.guild_id, user_id=id, reason=reason)
            await ctx.send(message)
            ...
            await self.client._http.remove_guild_ban(guild_id=ctx.guild_id, user_id=id)
            await ctx.send(message)
