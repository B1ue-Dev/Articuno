...
class Info(interactions.Extension):
	...
	async def _info_user(self, ctx: interactions.ComamndContext, user: str):
	...
	async def _info_server(self, ctx: interactions.CommandContext):
			guild = interactions.Guild(**await self.bot._http.get_guild(int(ctx.guild_id)), _client=self.bot._http)
