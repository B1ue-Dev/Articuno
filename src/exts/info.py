"""
This module is for Discord-related information commands about users, servers.
...
class Info(interactions.Extension):
    ...
    async def _info_user(self, ctx: interactions.CommandContext, user: str):
        ...
        fields = [
            ...
            interactions.EmbedField(
                name="Highest role", value=f"{role.mention}", inline=True
            ),
            interactions.EmbedField(
            ...
            interactions.EmbedField(
                name="Early supporter", value=f"{early_supporter}", inline=True
            ),
        ]
    ...
    async def _info_server(self, ctx: interactions.CommandContext):
        guild = interactions.Guild(**await self.client._http.get_guild(int(ctx.guild_id)), _client=self.client._http)
        user = interactions.User(**await self.client._http.get_user(int(guild.owner_id)), _client=self.client._http)
