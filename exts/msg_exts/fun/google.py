...
class Google(molter.MolterExtension):
    """Extension for Google related seach command."""

    def __init__(self, client: interactions.Client) -> None:
        self.client: interactions.Client = client

    @molter.prefixed_command(name="img")
    async def _img(self, ctx: molter.MolterContext, *, query: str):
                    ...
                    if int(res.user.id) == int(ctx.user.id):
                        if res.custom_id == "next":
                            ...
                            if _msg.content.isdigit() is False:
                                await res.send("That is not a valid number")
                            else:
